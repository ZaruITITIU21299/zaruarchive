...
export function TimelineForm({ timeline, onSave, onCancel }: TimelineFormProps) {
  ...
  const [form, setForm] = useState<TimelineFormData>({
    ...
    title_vi: '',
    description_en: timeline?.description ?? '',
    description_vi: '',
    cover_path: timeline?.coverPath ?? null,
    parent_timeline_id: timeline?.parentTimelineId ?? '',
    display_order: timeline?.displayOrder ?? 0,
  })

  const [saving, setSaving] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const set = (field: keyof TimelineFormData, value: unknown) =>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      >
        ...
        <ImageUpload
          ...
          storagePath={`${gameId}/timelines`}
          onUpload={(path) => set('cover_path', path)}
          className="mb-6"
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <TextArea label="Description (EN)" value={form.description_en} onChange={(v) => set('description_en', v)} rows={3} />
          <TextArea label="Description (VI)" value={form.description_vi} onChange={(v) => set('description_vi', v)} rows={3} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          />
          <Field
            label="Parent Timeline ID (optional)"
            value={form.parent_timeline_id}
            onChange={(v) => set('parent_timeline_id', v)}
          />
        </div>
